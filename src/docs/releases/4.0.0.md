...
`MenuItem.Id` can now be enforced as mandatory: a new `RequireMenuItemId` option (`OrchardCore_Navigation:NavigationOptions:RequireMenuItemId`, off by default) changes how `NavigationManager.BuildMenuAsync` merges and validates menu items. When enabled, items are merged by `Id` instead of by caption, so merging is no longer affected by localized/translated captions, and any menu item that still has no `Id` after merging is excluded and logged as an error. This is off by default so existing sites and third-party navigation providers are unaffected — merging still happens by caption and items without an `Id` are still included, exactly as before. Every built-in navigation provider in Orchard Core has been updated to always set an `Id`, using a bare, human-readable slug of its caption (e.g. `settings`, `queries`) rather than a module-qualified one, specifically so that same-captioned group nodes contributed by different modules (e.g. "Settings", "Queries") continue to merge into one node the way they already do today. This option is expected to default to `true` (and eventually be the only behavior) in a future version. Third-party navigation providers should start setting `NavigationItemBuilder.Id(...)` on every item they add.
